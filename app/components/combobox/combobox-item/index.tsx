...
export default function ComboboxItem({ item }: ComboboxItemProps) {
  ...
  return (
    <div
      ...
      className={`cursor-pointer pl-2 pr-1 overflow-hidden rounded-md flex justify-between items-center h-8 min-h-8 ${
        isActive ? 'bg-neutral-100' : 'bg-white'
      }`}
    >
      <div className="truncate text-slate-800 text-base">{item.label}</div>
