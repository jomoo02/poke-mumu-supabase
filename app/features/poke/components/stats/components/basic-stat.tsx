  ...
  return (
    <div className="flex px-1.5 gap-x-1 sm:gap-x-3 items-center h-8 first:rounded-t-lg">
      ...
      <div className="flex-1 flex items-center gap-x-1 md:gap-x-4 sm:px-2">
        <div className="min-w-9 max-w-9 text-center text-zinc-950">{value}</div>
        <StatBar value={value} max={maxStatValue} />
      </div>
      <div className="w-11 md:w-14 text-center text-zinc-950">{minStat}</div>
      <div className="w-11 md:w-14 text-center text-zinc-950">{maxStat}</div>
    </div>
  );
}
